...
from torch.nn import Flatten, Softmax, Linear, Module, CrossEntropyLoss, Dropout, LSTM, RNN
...
class Net(Module):
    ...
    def __init__(self, feat_dim, encode_len, n_classes, activation, lr):
        ...
        Args:
            ...
            encode_len (int): Number of time steps to encode.
        ...
        self.output = Linear(
            in_features=self.lstm2.hidden_size * self.encode_len,
    ...
    def forward(self, X):
        ...
        c0_1 = torch.zeros(
            1, batch_size, self.lstm1.hidden_size).to(self.device)
        h0_2 = torch.zeros(
            1, batch_size, self.lstm2.hidden_size).to(self.device)
        c0_2 = torch.zeros(
            1, batch_size, self.lstm2.hidden_size).to(self.device)
